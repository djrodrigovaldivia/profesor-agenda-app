...
export const ClaseModal: React.FC<ClaseModalProps> = ({
  ...
  const [tipo, setTipo] = useState<TipoClase>("dj");
  const [fecha, setFecha] = useState("");
  const [horaInicio, setHoraInicio] = useState("18:00");
  const [horaFin, setHoraFin] = useState("19:30");
  const [tema, setTema] = useState("");
  const [notas, setNotas] = useState("");
  const [estado, setEstado] = useState<"programada" | "realizada" | "cancelada">("programada");

  // Notification reminder state per class
  const [recordatorioActivo, setRecordatorioActivo] = useState(false);
  const [recordatorioValor, setRecordatorioValor] = useState(30);
  const [recordatorioUnidad, setRecordatorioUnidad] = useState<UnidadTiempoRecordatorio>("minutos");
  ...
  const alumnosDisponibles = useMemo(() => {
    if (editingClaseId && alumnoId) {
      const assigned = alumnos.find((a) => a.id === alumnoId);
      if (assigned && !assigned.activo) {
        return [assigned, ...alumnosActivos];
      }
    }
    return alumnosActivos;
  }, [alumnos, alumnosActivos, editingClaseId, alumnoId]);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (claseToEdit) {
        ...
        setTipo(claseToEdit.tipo);
        setFecha(claseToEdit.fecha);
        setHoraInicio(claseToEdit.horaInicio);
        setHoraFin(claseToEdit.horaFin);
    ...
    try {
      ...
      setSubmitError(
        err instanceof Error
          ? err.message
          : "Ocurrió un error inesperado al guardar la clase. Intenta nuevamente."
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setSubmitError(
        err instanceof Error
          ? err.message
          : "Ocurrió un error inesperado al eliminar la clase. Intenta nuevamente."
      );
      setShowDeleteConfirm(false);
    } finally {
      setIsDeleting(false);
  ...
  const isEditing = !!editingClaseId;
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {submitError && (
            <div
              ...
              className="p-3.5 rounded-xl bg-rose-950/60 border border-rose-800 text-rose-200 text-xs flex items-start gap-2.5"
            >
              <AlertCircle className="w-4 h-4 text-rose-400 shrink-0 mt-0.5" />
              <div className="flex-1 leading-relaxed">
                <p className="font-semibold text-rose-100">Error en la operación</p>
                <p className="text-rose-300/90 mt-0.5">{submitError}</p>
              </div>
            </div>
          )}
